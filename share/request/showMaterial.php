...
/*if (file_exists('../plugins/omega.class.php')){
    ...
    $o_files        = $omega_files->getFiles('kompassValue=09040101');
    if (isset($files)){
    ...
    for($i = 0; $i < count($files); $i++) {
        if ($files[$i]->file_context >= $file_context){
            ...
            if ($files[$i]->file_version == false){
                echo '<div class="'.ltrim ($files[$i]->type, '.').'_btn floatleft" style="width:70px;"></div>';
                        ...
                        if (checkCapabilities('file:showHits', $USER->role_id, false)){
                            echo '<span class="floatright space-right materialtxt_small">'.$files[$i]->hits.' Aufrufe</span><br>';
                        }
                ...
                foreach ($files[$i]->file_version as $key => $value) {
                    ...
                    if ($key == 't'){
                        if ($files[$i]->type == 'omega'){
                            echo '<div class="floatleft" style="width:70px;"><img  style="width:50px;" class="gray-border" src="'.$filename .'" alt=""></div>';
                        } else {
                            echo '<div class="floatleft" style="width:70px;"><img class="gray-border" src="'.$CFG->curriculum_path.$files[$i]->path.$filename .'" alt=""></div>';
                        }
                        $icon ++;
                    }   
                }
                   ...
                   echo '<lable></label><p>'.$files[$i]->description .' &nbsp;</p></div>'; // Leerzeichen  &nbsp; wichtig bei fehlender Beschreibung sonst wird es falsch dargestellt 
                    ...
                    if ($files[$i]->type == '.mp3'){
                        echo '<div class="materialtxt" onclick="updateFileHits('.$files[$i]->id.')"><p><a href="'.$CFG->curriculum_path.$files[$i]->path. $files[$i]->filename .'" target="_blank">'.$files[$i]->title.'</a><br>
                            <audio controls >
                            <source src="'.$CFG->access_file_url.'curriculum/'.$files[$i]->path. $files[$i]->filename.'" type="audio/mpeg">
            ...
            if (isset($files[$i+1])){
                if ($files[$i+1]->file_context == $files[$i]->file_context){ // Linie nur ausgeben, wenn nächstes Material im gleichen file_context
                    echo '<div class="materialseperator"></div><div class="space-top"></div>';
