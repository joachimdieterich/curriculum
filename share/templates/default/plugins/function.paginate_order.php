 ...
 * File:        function.paginate_order.php
...
function smarty_function_paginate_order($params, &$smarty) {
    $_id = 'default';
    $_attrs = array();
    
    if (!class_exists('SmartyPaginate')) {
        $smarty->trigger_error("paginate_order: missing SmartyPaginate class");
        return;
    }
    if (!isset($_SESSION['SmartyPaginate'])) {
        $smarty->trigger_error("paginate_order: SmartyPaginate is not initialized, use connect() first");
        return;        
    }
        ...
        switch($_key) {
            case 'id':
                if (!SmartyPaginate::isConnected($_val)) {
                    $smarty->trigger_error("paginate_order: unknown id '$_val'");
                    return;        
                }
                $_id = $_val;
                break;
    ...
    if (SmartyPaginate::getTotal($_id) === false) {
        $smarty->trigger_error("paginate_order: total was not set");
        return;        
    }
    ...
    $_text = isset($params['text']) ? $params['text'] : '_SET_TEXT_';
    $_url .= (strpos($_url, '?') === false) ? '?' : '&';
    
    if(($_item = SmartyPaginate::getCurrentItem($_id)) !== false) {
        $_url .= SmartyPaginate::getUrlVar($_id) . '=' . $_item;
    } 
    
    $sort       = SmartyPaginate::getSort('sort', $_id);
    if ($sort == 'ASC'){
        $sort = 'DESC';
    } else {
        $sort = 'ASC';
